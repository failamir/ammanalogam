	...
	function parse( $file ) {
		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			if ( 'SimpleXML_parse_error' == $result->get_error_code() ) {
				...
				echo $error[0] . ':' . $error[1] . ' ' . esc_html( $error[2] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			echo '</pre>';
...
class WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		if ( version_compare( PHP_VERSION, '8.0.0', '>=' ) ) {
			if ( function_exists( 'entity_loader' ) ) {
				$old_value = entity_loader( true );
			}
			$success = $dom->loadXML( file_get_contents( $file ) );
			if ( ! is_null( $old_value ) ) {
				entity_loader( $old_value );
			}
		} else {
