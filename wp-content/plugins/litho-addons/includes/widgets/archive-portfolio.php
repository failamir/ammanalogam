...
if ( ! is_section_builder_archive_portfolio_template() ) {
	return;
}
...
if ( ! class_exists( 'LithoAddons\Widgets\Archive_Portfolio' ) ) {
	...
	class Archive_Portfolio extends Widget_Base {
		...
		public function get_keywords() {
			return [ 'portfolio', 'masonry', 'grid', 'gallery', 'list', 'project', 'cpt', 'custom post type' ];
		}
		...
		protected function render( $instance = [] ) {
			...
			$this->add_render_attribute( 'wrapper', [
				'class' => [ 'portfolio-wrap', 'portfolio-grid-without-filter' ]
			] );
			...
			switch ( $portfolio_style ) {
				...
				default:
					$this->add_render_attribute( 'wrapper', [
						'class'                   => [ 'portfolio-grid', 'grid', $loader_class, $litho_column_class_list, $litho_portfolio_id ],
						'data-uniqueid'           => $litho_portfolio_id,
						'data-portfolio-settings' => json_encode( $dataSettings )
					] );
					break;
			...
			if ( $wp_query->have_posts() ) {
				?><div <?php echo $this->get_render_attribute_string( 'main_wrapper' ); ?>><?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
															...
															if ( 'yes' === $settings['litho_portfolio_show_custom_link'] && ! empty( $settings['litho_portfolio_custom_link_icon']['value'] ) ) {
																...
																<?php
																	if ( $is_new || $custom_link_icon_migrated ) {
																		Icons_Manager::render_icon( $settings['litho_portfolio_custom_link_icon'], [ 'aria-hidden' => 'true' ] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
																	} else {
											...
											<figcaption>
												<div class="portfolio-hover d-flex">
													<div class="portfolio-caption scale"><?php
														...
														if ( 'yes' === $portfolio_show_post_subtitle && $litho_subtitle ) {
															?><div class="subtitle">
																<span><?php echo esc_html( $litho_subtitle ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  ?></span>
															</div><?php
														}
													?></div>
												</div>
						...
						if ( ! empty( $porfolio_pagination_type ) && $wp_query->max_num_pages > 1 ) {
							...
							switch ( $porfolio_pagination_type ) {
								...
								case 'infinite-scroll-pagination':
									...
									?><div <?php echo $this->get_render_attribute_string( 'pagination_main' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php
										if ( get_next_posts_link( '', $wp_query->max_num_pages ) ) {
											next_posts_link( '<span class="old-post">'. esc_html__( 'Older Portfolio', 'litho-addons' ). '</span><i class="fas fa-long-arrow-alt-right"></i>', $wp_query->max_num_pages );
										}
									?></div><?php
								break;
								case 'load-more-pagination':
		...
		public function litho_get_open_lightbox_link( $index ) {
			...
			if ( ! empty( $settings['litho_portfolio_lightbox_icon']['value'] ) ) {
				$open_lightbox_link .= '<a ' . $this->get_render_attribute_string( $link_key ) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					if ( $is_new || $lightbox_icon_migrated ) {
						ob_start();
							Icons_Manager::render_icon( $settings['litho_portfolio_lightbox_icon'], [ 'aria-hidden' => 'true' ] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
