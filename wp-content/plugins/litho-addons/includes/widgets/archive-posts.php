...
if ( ! is_section_builder_archive_template() ) {
	return;
}
	...
	class Archive_Posts extends Widget_Base {
		...
		protected function register_controls() {
			...
			$this->start_controls_tabs( 'litho_image_overlay_tabs' );
				$this->start_controls_tab( 'litho_image_overlay_normal_tab', 
					[ 
						...
						'condition' => [ 
							'litho_blog_style!' => [ 'blog-classic', 'blog-metro', 'blog-overlay-image' ]
						],
					]
				);
					$this->add_control(
			...
			$this->start_controls_tabs( 'litho_blog_list_title_tabs' );
				$this->start_controls_tab( 'litho_blog_list_title_normal_tab', 
					[ 
						'label' => __( 'Normal', 'litho-addons' ),
						'condition' => [ 
							'litho_blog_style!' => [ 'blog-overlay-image' ]
						]
					]
				);
					$this->add_control(
			...
			$this->start_controls_section(
				...
				[
					...
					'show_label'    => false,
					'condition'     => [ 
						'litho_blog_style!' => [ 'blog-masonry', 'blog-overlay-image', 'blog-simple', 'blog-standard' ],
						'litho_separator' => 'yes',
					], // NOT IN
				]
			);
		...
		protected function render( $instance = [] ) {
			...
			$this->add_render_attribute( 'wrapper', [
				'class' => [ 'grid', $litho_blog_style, $litho_column_class_list, $litho_pagination ],
			...
			if ( $wp_query->have_posts() ) {
				?>
				<ul <?php echo $this->get_render_attribute_string( 'wrapper' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>><?php
					if ( 'blog-side-image' !== $litho_blog_style ) {
					...
					while ( $wp_query->have_posts() ) :
						...
						$this->add_render_attribute( $blog_post_key, [
							'class' => [ 'blog-post', $litho_blog_list_content_box_alignment_main_class, $litho_blog_post_content_box_hover_animation ],
						] );
											...
											if ( ! post_password_required() && 'yes' === $litho_show_post_thumbnail && has_post_thumbnail() ) {
												...
												<div class="blog-post-images"><?php
													if ( 'gallery' === $post_format && '' === $litho_show_post_format ) {
														echo $this->litho_post_gallery_format( get_the_ID() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
											...
											<div class="post-details">
												...
												if ( 'yes' === $litho_show_post_read_more_button ) { ?>
													<div <?php echo $this->get_render_attribute_string( $button_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
														<a <?php echo $this->get_render_attribute_string( $button_link_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
															<span <?php echo $this->get_render_attribute_string( $button_content_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
																<span <?php echo $this->get_render_attribute_string( $button_text_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
